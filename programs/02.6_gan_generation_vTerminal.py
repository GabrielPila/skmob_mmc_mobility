            ...
            for genDim in generatorDims:
                for optimizer in optimizers:
                    for batch_size in batch_sizes:
                        for epoch in epochs:
                            hyperparams.append({
                                'dir_user_input': dir_user_input, 
                                'dir_user_output': dir_user_output,
                                'filename': filename,
                                'input_dim': input_dim,
...
if __name__ == '__main__':
    ...
    hyperparams = get_hiperparam_combinations()
                    
    for hparam in tqdm(hyperparams):

        train_gan(
            path_data = os.path.join(PATH_LOCAL_DATA, hparam['dir_user_input']),
            path_output = os.path.join(PATH_LOCAL_DATA, hparam['dir_user_output']),
            filename = hparam['filename'],
